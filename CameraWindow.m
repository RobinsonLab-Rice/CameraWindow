...
persistent videoBuffer frameCount timeStamps;
...
%     data = getsnapshot(UD.video);
    ...
    timeStamps{frameCount} = event.Timestamp;
    ...
    if frameCount > 0
        ...
        frameCount = 0;
        start(timer('TimerFcn',{@saveVid,data,times}));
    end
end
end

% Timer Function for saving video
function saveVid(obj,~,data,varargin)
[~,h] = MatPad;
if ~isempty(h)
    timestr = datestr(clock,'mm/dd/yy HH:MM AM');
...
if ~isempty(varargin{1})
    times = varargin{1}; %#ok<NASGU>
    save(fullname,'times','-mat');
end
